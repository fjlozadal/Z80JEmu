...
public class Processor {
    ...
    //Alternative Accumulator and flags
    private int regAx;
    private int regFx;
    ...
    // pinReset == true, when pin reset is active
    private boolean pinReset = false;
    private int memptr;

    
    /* Some flags are precalculated for faster processing
     * Specifically, SIGN, ZERO, bit 3, bit 5, PARITY and ADDSUB:
     * sz53n_addTable has the ADDSUB flag at 0 and parity without calculating
     * sz53pn_addTable has the ADDSUB flag at 0 and parity calculated
     * sz53n_subTable has the ADDSUB flag at 1 and parity without calculation
     * sz53pn_subTable has the ADDSUB flag at 1 and parity calculated
     * The remaining bits are at 0 in the four tables what is
     * important for many operations that set certain flags to 0.
     *  If you put it to 1 by the same method just do an OR with
    ...
    //True in an address means that instruction in the address is going to be
    //executed 
    private final boolean breakpointAt[] = new boolean[65536];

    // Constructor
    public Processor(Memory memory, NotifyOps notify) {
        MemIoImpl = memory;
        NotifyImpl = notify;
        execDone = false;
    ...
    public void setMemIoHandler(Memory memIo) {
        MemIoImpl = memIo;
    }
    ...
    // 8 bits alternative registers access
    ...
    /* Functions incRegXX y decRegXX optimized
     * 
     */
    ...
    // Specific purpose registers access
    ...
    // access for (hidden) MEMPTR register
    ...
    // Acess for Interruption Flip-flops
    ...
    public final ProcessorState getProcessorState() {
    ...
    public final void setProcesorState(ProcessorState state) {
        regA = state.getRegA();
    ...
    // Reset
    /* According to  Sean Young's document, which is in
     * [http://www.myquest.com/z80undocumented], the best way to emule a 
     * reset is switching to PC, IFF1, IFF2, R and IM0 = 0 and all the registers to
     * a 0xFFFF.
     *
